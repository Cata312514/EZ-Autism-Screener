...
if __name__ == '__main__':
    ...
    train_images = np.load(os.path.join(training_dir, 'train.rec'))['image']
    train_labels = np.load(os.path.join(training_dir, 'train.rec'))['label']
    val_images  = np.load(os.path.join(validation_dir, 'val.rec'))['image']
    val_labels  = np.load(os.path.join(validation_dir, 'val.rec'))['label']
    
    
    K.set_image_data_format('channels_last')

    # Adding batch dimension to the input
    train_images = train_images.reshape(train_images.shape[0], 100, 100, 3)
    val_images = val_images.reshape(val_images.shape[0], 100, 100, 3)
    ...
    data_augmentation = Sequential([tf.keras.experimental.preprocessing.RandomFlip("horizontal"),])
